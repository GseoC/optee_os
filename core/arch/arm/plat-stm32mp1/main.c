...
static TEE_Result init_stm32mp1_drivers(void)
{
	...
	struct firewall_conf firewall_conf = { };
	int node = 0;
	...
	query->ctrl = dt_driver_provider_priv_data(prov);
	firewall_conf.queries = query;
	firewall_conf.nb_queries = 1;

	/* Secure SYSRAM */
	res = firewall_set_memory_configuration(&firewall_conf, SYSRAM_BASE,
						SYSRAM_SIZE);
	if (res)
